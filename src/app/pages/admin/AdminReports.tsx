...
import { Download, TrendingUp, TrendingDown, Users, Building2, Droplet, Calendar } from "lucide-react";

const REGIONAL_DATA = [
  { region: "Centre", hospitals: 18, donors: 4521, donations: 672, growth: 12 },
  { region: "Hauts-Bassins", hospitals: 8, donors: 2134, donations: 298, growth: 8 },
  { region: "Cascades", hospitals: 4, donors: 892, donations: 124, growth: -3 },
  { region: "Nord", hospitals: 6, donors: 1245, donations: 168, growth: 5 },
  { region: "Est", hospitals: 3, donors: 734, donations: 89, growth: 15 },
  { region: "Sud-Ouest", hospitals: 3, donors: 395, donations: 53, growth: -1 }
];

const MONTHLY_TRENDS = [
  { month: "Oct", donations: 980, donors: 7842 },
  { month: "Nov", donations: 1020, donors: 8124 },
  { month: "Déc", donations: 890, donors: 8315 },
  { month: "Jan", donations: 1150, donors: 8654 },
  { month: "Fév", donations: 1080, donors: 8821 },
  { month: "Mar", donations: 1240, donors: 8921 }
];

export function AdminReports() {
  const totalDonations = REGIONAL_DATA.reduce((acc, r) => acc + r.donations, 0);
  const totalDonors = REGIONAL_DATA.reduce((acc, r) => acc + r.donors, 0);
  const totalHospitals = REGIONAL_DATA.reduce((acc, r) => acc + r.hospitals, 0);
  const avgGrowth = (REGIONAL_DATA.reduce((acc, r) => acc + r.growth, 0) / REGIONAL_DATA.length).toFixed(1);

  const maxDonations = Math.max(...MONTHLY_TRENDS.map(m => m.donations));
